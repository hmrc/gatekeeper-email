...
import uk.gov.hmrc.gatekeeperemail.repositories.MongoFormatter.emailFormatter
...
class EmailRepository @Inject()(mongoComponent: MongoComponent, appConfig: AppConfig)
  ...
  extends PlayMongoRepository[Email](
    ...
    indexes = Seq(IndexModel(ascending("composedBy", "createDateTime"),
      IndexOptions().name("composedEmailIndex").background(true))
    )
  ) {
