...
object TopicOptionChoice {
  def apply(text: String): Option[TopicOptionChoice] = TopicOptionChoice.values.find(_.toString().equalsIgnoreCase(text))
  ...
  given Format[TopicOptionChoice] = SimpleEnumJsonFormatting.createEnumFormatFor[TopicOptionChoice]("Topic Option Choice", apply)
}
